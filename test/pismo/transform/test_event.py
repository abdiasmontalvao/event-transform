...
class TestEventTransform:
  ...
  SOURCE_PATH = f'{pathlib.Path().resolve()}/test/ingest_mock/'
  DESTINATION_PATH = f'{pathlib.Path().resolve()}/test/temp_output_bucket'

  def test_if_is_filtering_by_type_correctly(self):
    event_transform = EventTransform(self.SOURCE_PATH + 'mock-3-type-12-events.json', self.DESTINATION_PATH)
    expected_quant = 4
  ...
  def test_if_remove_duplicateds_works_and_preserves_the_latest_event(self):
    event_transform = EventTransform(self.SOURCE_PATH + 'mock-repeateds.json', self.DESTINATION_PATH)
  ...
  def test_if_remove_duplicateds_works_and_preserves_the_latest_event_2(self):
    event_transform = EventTransform(self.SOURCE_PATH + 'mock-repeateds-2.json', self.DESTINATION_PATH)
